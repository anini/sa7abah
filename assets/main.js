var Sa7abah = {
	...
	all_letters: ['أ','ب','ت','ث','ج','ح','خ','د','ذ','ر','ز','س','ش','ص','ض','ط','ظ','ع','غ','ف','ق','ك','ل','م','ن','ﮬ','و','ي'],
...
function start(){
	Sa7abah.Helpers.trails=0;
	Sa7abah.Helpers.c=0;
	Sa7abah.Helpers.i=getRand();
	Sa7abah.Helpers.name=getName();
	Sa7abah.Helpers.letters=Sa7abah.Helpers.name.split("");
	Sa7abah.Helpers.s=(Sa7abah.Helpers.name.replace(/\s/g, '')).length;
	fillLetters();
	fillKeyboard();
}
function getRand(){
	return Math.floor(Sa7abah.Helpers.t*Math.random()%151);//Math.floor((Math.random()*479));
}
function getName(){
	return Sa7abah.companions[Sa7abah.Helpers.i];
}
function fillLetters(){
	$('#guess').html('الصحابي الجليل: ');
	for(var i=0; i<Sa7abah.Helpers.letters.length; i++){
		if(Sa7abah.Helpers.letters[i]==' '){
...
function fillKeyboard(){
	$('#keyboard').html('');
	for(var i=0; i<Sa7abah.all_letters.length; i++){
		$('#keyboard').append('<div id="k'+(i+1)+'" onclick="checkLetter(\''+Sa7abah.all_letters[i]+'\'); return false;">'+Sa7abah.all_letters[i]+'</div>');
	};
}
function checkLetter(l){
	if(Sa7abah.Helpers.trails==6)return false;
	var match=0;
	if(l=='ﮬ')l='ه';
	for(var i=0; i<Sa7abah.Helpers.letters.length; i++){
		if( Sa7abah.Helpers.letters[i]==l 				||
		 	(l=='أ' && Sa7abah.Helpers.letters[i]=='ا')	||
		 	(l=='أ' && Sa7abah.Helpers.letters[i]=='إ')	||
		 	(l=='أ' && Sa7abah.Helpers.letters[i]=='ء')	||
		 	(l=='أ' && Sa7abah.Helpers.letters[i]=='ئ')	||
		 	(l=='و' && Sa7abah.Helpers.letters[i]=='ؤ')	||
		 	(l=='ي' && Sa7abah.Helpers.letters[i]=='ى')	||
		 	(l=='ه' && Sa7abah.Helpers.letters[i]=='ة') ) {
				$('#l'+(i+1)).html(Sa7abah.Helpers.letters[i]);
				match++;
				Sa7abah.Helpers.c++;
		}
	};
	if(l=='ه')l='ﮬ';
	if(!match){
		$('#s'+(Sa7abah.Helpers.trails+2)).slideDown();
		Sa7abah.Helpers.trails++;
		$('#face').addClass('e'+Sa7abah.Helpers.trails);
		$('#k'+(Sa7abah.all_letters.indexOf(l)+1)).addClass('no');
		$('#dialog').html('لا يوجد تطابق للحرف ”'+l+'“');
	}else{
		$('#k'+(Sa7abah.all_letters.indexOf(l)+1)).addClass('yes');
		var m=Sa7abah.numbers[match]+' '+'تطابقات';
		if(match==1)m='تطابق';
		else if(match==2)m='تطابقين';
		$('#dialog').html('يوجد '+m+' للحرف ”'+l+'“');
		if(Sa7abah.Helpers.c==Sa7abah.Helpers.s){
			...
			setTimeout(function(){
				...
				$('#face').addClass('done');
				setTimeout(function(){
					$('#dialog').html('الصحابي الجليل '+Sa7abah.Helpers.name);
					$('#bio').html(Sa7abah.Helpers.bios[Sa7abah.Helpers.i]);
					$('#bio').slideDown('slow');
					setTimeout(function(){
	...
	$('#k'+(Sa7abah.all_letters.indexOf(l)+1)).addClass('dis');
	$('#k'+(Sa7abah.all_letters.indexOf(l)+1)).attr('onclick', '$("#dialog").html("لقد اخترت هذا الحرف من قبل");return false;');
	if(Sa7abah.Helpers.trails==6){
		$('#keyboard div').attr('onclick', 'return false');
		setTimeout(function(){
			$('#dialog').html('لم تحزر');
			setTimeout(function(){
				fillName();
				$('#dialog').html('الاسم الصحيح: '+Sa7abah.Helpers.name);
				$('#bio').html(Sa7abah.Helpers.bios[Sa7abah.Helpers.i]);
				$('#bio').slideDown('slow');
				setTimeout(function(){
					removeEmotions();
...
function removeEmotions(){
	for(var i=Sa7abah.Helpers.trails; i>0; i--){
